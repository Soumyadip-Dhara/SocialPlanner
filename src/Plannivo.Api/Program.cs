using Plannivo.Api.Extensions;
using Plannivo.Api.Middleware;
using Plannivo.Services;
...
builder.Host.UseSerilog((context, services, configuration) => configuration
    ...
    .WriteTo.Console()
    .WriteTo.File("logs/plannivo-.log", rollingInterval: RollingInterval.Day));
...
if (app.Environment.IsDevelopment())
{
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "Plannivo API v1");
    });
}

app.UseSerilogRequestLogging();
app.UseHttpsRedirection();
app.UseCors("PlannivoPolicy");
